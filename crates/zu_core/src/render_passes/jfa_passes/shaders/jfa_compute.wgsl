...
@group(0) @binding(0) var input_texture: texture_storage_2d<rg16float, read>;
@group(1) @binding(0) var output_texture: texture_storage_2d<rg16float, write>;
...
fn fs_main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let current_value = textureLoad(input_texture, vec2<i32>(pixelCoord));
    ...
    for (var y: i32 = -1; y <= 1; y += 1) {
        for (var x: i32 = -1; x <= 1; x += 1) {
            ...
            let clampedCoord = clamp(sampleCoord, vec2<i32>(0), constants.texture_size);

            let sample_value = textureLoad(input_texture, clampedCoord);
